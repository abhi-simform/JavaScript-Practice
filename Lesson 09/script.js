...
console.log({ goalKeeper: gk, fieldPlayers: fieldPlayers });
...
game.odds.team1 < game.odds.team2 && console.log('Team 1 won');
...
console.log(`Average odd is: ${totalOdd / Object.values(game.odds).length}`);

for (const [name, value] of Object.entries(game.odds)) {
  console.log(`Odd of victory ${game[name] ?? 'draw'} is ${value}`);
}

const scored = {};
for (const value of Object.values(game.scored)) {
  scored[value] ? (scored[value] += 1) : (scored[value] = 1);
}
